...
import { Mask, SlidePage, Search, Empty, Loading } from '@boluome/oto_saas_web_app_component'
...
const CarType = (props) => {
  ...
  // console.log('CarType props-----------',props)

  const { hotBrandList = [], brands = [] } = props

  const { handleChooseId, handleChooseCar } = props
  ...
  return(
    <div>
          ...
          Mask(
            <SlidePage target='right' showClose={ false } >
              <Search
                ...
                onFeach = { (searchKey, cb) => search(searchKey, brands, cb) }
      ...
      <div className='mainContainer'>
        <div className='hotBrand' id={ `letterhot` }>
          <h3>热门品牌</h3>
          <div className='hotBrandContainer'>
            {
              hotBrandList.map(({ brandIcon, brandId, brandName },index) => (
                <div key={ `hotBrandKey${ brandId }` } className='hotBrandBox'
                  onClick={ () => showDetail(brandId, brandIcon, brandName) }>
                  <img src={ brandIcon } />
                  <span>{ brandName }</span>
                </div>
              ))
        ...
        <div className='normalBrands'>
          {
            brands.length > 0 ? (
              brands.map(({ key, data }, index) => {
                ...
                return(
                  <div key={ `brandKey${ index }` } id={ `letter${ letter }` } className='brandContainer'>
                    <div className='brandsKey'>{ letter }</div>
                    <ul>
                      {
                        data.map(({ brandIcon, brandId, brandName }, index) => (
                          <li key={ `brandIdKey${  Math.random() }` } className='brandBox'
                            onClick={ () => showDetail(brandId, brandIcon, brandName) }>
                            <img src={ brandIcon } />
                            <span>{ brandName }</span>
                          </li>
...
// 搜索请求通过组件方式传入
const search = (searchKey, brands, cb) => {
  // const handleClose = Loading()
  if(!searchKey) return
  cb(null, brands.reduce((arr, curr) => [...arr, ...curr.data], [])
