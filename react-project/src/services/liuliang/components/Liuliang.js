...
import '../styles/liuliang.scss'

const Liuliang = ({ hfInfo = { isp: '' }, isFirstIptClick, handleIptClick,
 ...
 handlePromotionChange, curDiscountData,
  ...
  let hfList = [['20M'], ['50M'], ['100M'], ['200M'], ['500M']].map(item => ({ title: `${ item[0] }`, discPrice: '' }))
  let curHfIndex = '100'
  let curHfArea = ''
  let curHfPrice = ''
  let isSubmit = false
  ...
  if (isp) {   // 当请求回数据后
    hfList = list.map(item => ({ title: item.flow, discPrice: `售价 ${ item.price }元` }))
    curHfIndex = 0
    curHfArea = area + isp
    ...
    <div className='hf'>
      {
        ...
        (
          <div>
            <UserCenter categoryCode='liuliang' />
          </div>
        )
      }
      <div className='header'>
        <LiuliangItem { ...{ curPhone, handleIptChange, isFirstIptClick, handleIptClick, phoneHistorys, visibilityHistory, handlePhoneHistory, handleClearHistory, handleClearPhone, changeFocus, changeBlur, curHfArea } } />
      </div>
      <WhiteSpace />
      <div className='container'>
        {
          hfList.map((item, index) => (
            <div
              ...
              style={{ WebkitJustifyContent: 'center', WebkitAlignItems: 'center', WebkitFlexDirection: 'column', border: '1px solid #ccc' }}
      ...
      <div className='footer'>
        <p className='pay-price'>
          ...
          { curHfPrice && !discount && <del className='real-price'>{ `¥ ${ price.toFixed(2) }` }</del> }
        ...
        {
          curHfPrice &&
          <ExchangeActivePopup orderType='liuliang' channel='dhst' amount={ curHfPrice } popupStyle={{ top: '-1.16rem' }} promotionCallback={ handlePromotionChange } />
        }
      </div>
    </div>
  )
...
class LiuliangItem extends React.Component {
  constructor(props) {
    super(props)
    this.state = { value: '' }   // 给input框设置一个state
  ...
  handleChange(value) {
    ...
    setTimeout(() => {
      this.setState({ value })
      this.props.handleIptChange(value, prePhone) // 调用父组件传过来的参数
    }, 0)
  }
  ...
  handleBlur() {
    // 如果不加seTimeout会造成，点击历史，不触发onclick事件
    setTimeout(() => {
  ...
  handleIptClickMid() {
    const { isFirstIptClick = true, handleIptClick } = this.props
    if (isFirstIptClick) handleIptClick()
    document.querySelector('.input input').focus()
  }
  ...
  render() {
    const { curPhone, phoneHistorys, visibilityHistory, handlePhoneHistory, handleClearHistory, curHfArea } = this.props

    const { value, showClear = false } = this.state
    ...
    return (
      <div className='number'>
        <InputItem
          ...
          onClick={ () => this.handleIptClickMid() }
        ...
        <div className='icon-container' onClick={ this.handleIconClick } />
