  ...
  render() {
    ...
    if (dataList) {
      for (let i = 0; i < dataList.length; i++) {
        if (keys && (dataList[i].dateStr === keys)) {
          index = i
        }
      }
    }
    const dataLi = dataList && dataList.map((o, i) => <span ref={ node => { if (node) { this.tabSpan[i] = node } } } key={ `${ i + 1 }` } className='select_li' onClick={ () => this.handleSelect(i, o) }>{ o.dateStr }<br />{ o.eventWeekday }</span>)
    return (
